...
for skill in a11y agent-safety ansible powershell-module-engineering; do
  ...
  cp ".agents/skills/$skill/SKILL.md" ~/.agents/skills/"$skill/SKILL.md"
done
...
foreach ($skillName in @('a11y', 'agent-safety', 'ansible', 'powershell-module-engineering')) {
    ...
    Copy-Item -LiteralPath "./.agents/skills/$skillName/SKILL.md" -Destination $skillDestination -Force
}
...
reviewed skill outputs generated by the project installation. The compiler
commands ensure that both CLIs receive the same small personal instruction
core; specialist skill bodies remain on demand.
