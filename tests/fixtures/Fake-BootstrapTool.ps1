...
if ($ToolArgument.Count -gt 0 -and $ToolArgument[0] -ceq 'install') {
    ...
    $null = New-Item -ItemType Directory -Path (Join-Path $homePath '.codex') -Force
    $null = New-Item -ItemType Directory -Path (Join-Path $homePath '.copilot') -Force
    Write-DefaultMcpState
    Add-Content -LiteralPath (Join-Path $homePath '.codex/config.toml') -Value @(
        '[mcp_servers.github-mcp-server]'
        'url = "https://api.githubcopilot.com/mcp/"'
        'bearer_token_env_var = "GITHUB_TOKEN"'
    )
    Set-Content -LiteralPath (Join-Path $homePath '.copilot/mcp-config.json') `
        -Value '{"servers":{"github-mcp-server":{"url":"https://api.githubcopilot.com/mcp/"}}}'
    exit 0
}
